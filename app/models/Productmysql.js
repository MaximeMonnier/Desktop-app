const { getConnection } = require('../databasemysql');
...
class Product {
  ...
  static async findAll() {
    try {
      ...
      // requete à la db
      const results = await connect.query("SELECT * FROM stockage ORDER BY id DESC");
      // si on a des résultats, on boucle sur le tableau reçu
      // la méthode map permet de retourner un tableau
      // On retourne ici un tableau d'instance de la classe Product générer via les données des résultats
  ...
  static async getOneById(id) {
    try {
      const connect = await getConnection();
      const results = await connect.query("SELECT * FROM stockage WHERE id=?", id);
  ...
  async addOne() {
    try {
      const connect = await getConnection();
      const results = await connect.query("INSERT INTO stockage SET ?", this);
      return null;
    } catch (error) {
      console.log(error);
      if (error.detail) {
  ...
  async edit() {
    try {
      const connect = await getConnection();
      const result = await connect.query("UPDATE stockage SET ? WHERE id=?",[this, this.id]);
      return result;
    } catch (error) {
      console.log(error);
      if (error.detail) {
